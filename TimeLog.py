...
def get_db_connection(db_path='TimeLogAppInfor.db'):
...
def create_table():
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS Activity_Infor (
...
def add_Activity(Activity_Name, Description):
    ...
    conn = sqlite3.connect('TimeLogAppInfor.db')
    c = conn.cursor()
    c.execute ('INSERT INTO Activity_Infor ( Activity_Name, Description, Start_time) VALUES (?, ?, ?)', (Activity_Name, Description, Start_time ))
...
def get_Activity():
    conn = sqlite3.connect('TimeLogAppInfor.db')
    c = conn.cursor()
    c.execute('SELECT Activity_id, Activity_Name, description, Start_time FROM Activity_Infor ')
    Activity = c.fetchone()
    conn.close()
    return Activity
...
def main():
    ...
    if st.button('Start'):
        ...
        print(f"activity_id after add_Activity: {Activity_id}") 
